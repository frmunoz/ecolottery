...
# The pond macroinvertrebrate dataset 

We will now analyze macroinvertebrate communities in ponds of different area [@jeliazkov_scale-effects_2013]. It includes 75 communities with 82 plant species.

```{r get.jeliazkov}
data <- LSmatred$Jeliazkov2013
```

In the following, we will assess how pond area can affect stochastic population dynamics and the migration-drift equilibrium in the *coalesc* model of community assembly.

```{r jeliazkov.pool}
pool <- data.frame(ind = as.character(1:sum(data$comm)), sp = rep(colnames(data$comm),colSums(data$comm)))
```

## ABC-based inference of community assembly parameters

For simplicity sake, we consider in this case neutral community dynamics depending solely on the immigration parameter *m*.
...
res.tab <- list()
for(i in 1:nrow(data$comm))
  res.tab[[i]] <- coalesc_abc(data.frame(ind=1:sum(data$comm[i,]),sp=rep(colnames(data$comm),data$comm[i,])), pool, multi = "single", f.sumstats = f.sumstats,
                   nb.samp = nb.samp, parallel = TRUE, tol = 0.5, method.abc = "neuralnet",
                   pkg = "vegan")
...
```{r abc.inf, eval=F}
res.abc <- abc(f.sumstats(data$comm,data.frame(row.names=colnames(data$comm),trait.pca$li[colnames(data$comm),1])), param=res.tab$par, sumstat=res.tab$ss, tol=0.5, method="loclinear")
plot(res.abc, param=res.tab$par)
```

```{r m.space}
m.comm <- lapply(res.abc, function(x mean(x$abc$unadj.val[,1])))
plot(m.comm, data$envir$NetArea)
```
...
res.tab <- coalesc_abc(data$comm, pool, multi = "tab", f.sumstats = f.sumstats,
...
```
The more simulations are performed, the more robust the inference can be.
Threfore, we propose to gather the results of simulations from the participants and merge them for subsequent ABC analysis.
...
```{r m.space}
m.comm <- lapply(res.abc, function(x mean(x$abc$unadj.val[,3])))
plot(m.comm, data$coord[,1])
plot(m.comm, data$coord[,2])
```
