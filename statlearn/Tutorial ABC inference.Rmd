...
```{r load.jeliazkov}
load("res.tab.Jeliazkov.RData")
```

```{r m.space.jeliazkov}
m.comm <- unlist(lapply(res.tab, function(x) ifelse(class(x$abc)!="abc",NA,median(x$abc$unadj.values[,1],na.rm=T))))
plot(m.comm, data$envir$UrbanGradM)
```
