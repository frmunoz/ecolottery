coalesc <- function(J, theta, m = 1, filt = NULL, pool = NULL, Jpool = 50*J)
{
  ...
  else if(ncol(pool) < 2) {stop("The regional pool is misdefined (at least two columns required)")}
  else if(ncol(pool) == 2) {warning("No trait information provided in the regional pool")}
  
  if(!is.null(filt)) {
    filt1 <- function(x) filt(x)
  }
  else {filt1 <- function(x) sapply(x, function(x) 1)}
