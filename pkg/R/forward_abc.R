forward_abc <- function(comm.obs, pool = NULL, traits = NULL, multi = "single", initial, d = 1, gens = 150, limit.sim = FALSE, coeff.lim.sim = 1,
                            sigm = 0.1, filt.abc = NULL, prob.death = NULL,
                            method.dist = "euclidean", add = F, var.add = NULL,
                            ...
                            parallel = TRUE, tol = NULL, pkg = NULL, method = "rejection")
{
  ...
  if (!requireNamespace("abc", quietly = TRUE)) {
    stop("forward_abc requires package abc to be installed")
  }
  ...
  sim <- do.simul.forward(J, pool, multi, prop, nb.com, traits, f.sumstats, filt.abc, add, var.add, 
...
do.simul.forward <- function(J, pool = NULL, multi = "single", prop = F, nb.com = NULL,
        ...
        if(multi == "tab") {
          ...
          for (i in 1:nb.com) {
            try({
              ...
              # Initial state is a random draw from the pool
              initial <- rmultinom(1, J.loc, prob = table(pool$sp))
              # Need to include add and var.add in forward
              # Include traits argument in forward?
              comm.samp <- ecolottery::forward(initial = initial, prob = params.samp[length(params.samp)],
                                               d = d, gens = gens, pool = pool,
                                               limit.sim = limit.sim, coeff.lim.sim = coeff.lim.sim,
                                               sigm = sigm, filt = filt, prob.death = prob.death,
                                               method.dist = method.dist, add = add, var.add = var.add[i,])
