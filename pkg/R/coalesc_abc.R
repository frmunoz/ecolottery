coalesc_abc <- function(comm.obs, pool, multi = F, traits=NULL, f.sumstats, filt.abc, params, nb.samp = 10^6, parallel = T, tol = 1*10^-4, pkg = NULL, method="neuralnet")
{
  ...
  if(multi) 
  {
    ...
    for(i in unique(comm.obs[,1]))
        if(is.null(traits)) stats.obs <- rbind(stats.obs, f.sumstats(comm.obs[comm.obs[,1]==i,3])) else
      stats.obs <- rbind(stats.obs, f.sumstats(comm.obs[comm.obs[,1]==i,3],traits[comm.obs[comm.obs[,1]==i,3],]))
  } else
  if(is.null(traits)) stats.obs <- f.sumstats(comm.obs) else
  stats.obs <- f.sumstats(comm.obs[,2],traits[comm.obs[,2],])
  
  # Community simulation
  sim <- do.simul(J, pool, traits, f.sumstats, filt.abc, params, nb.samp, parallel, tol, pkg, method)
   
  # ABC estimation
  colnames(sim$stats) <- names(stats.obs)
  sim$stats.scaled <- sapply(1:ncol(sim$stats), function(y) sim$stats[,y]/max(abs(sim$stats[,y]),na.rm=T))
...
do.simul <- function(J, pool, traits=NULL, f.sumstats, filt.abc, params, nb.samp = 10^6, parallel = T, tol = 1*10^-4, pkg = NULL, method="neuralnet")
{
  ...
  prior <- c()
  for(i in 1:nrow(params)) prior[[i]]=runif(nb.samp, min = params[i,1], max = params[i,2])              
  ...
  {
    force(J); force(pool); force(traits); 
    force(filt.abc); force(f.sumstats)
    force(prior); force(pkg)
    summCalc <- function(j,traits,prior,J,pool,filt.abc,f.sumstats)
    {
      params.samp <- unlist(lapply(prior,function(x) x[j]))
      stats.samp <- NA
      try({
        comm.samp <- coalesc(J, m=params.samp[length(params.samp)], filt = function(x) filt.abc(x,params.samp), pool=pool, traits=traits);
  ...
  if(parallel) models <- parallel::parLapply(parallelCluster,1:nb.samp,mkWorker(traits,prior,J,pool,filt.abc,f.sumstats,pkg)) else
    models <- lapply(1:nb.samp,mkWorker(traits,prior,J,pool,filt.abc,f.sumstats,pkg))
  #models <- c(); for(i in 1:nb.samp) models[[i]]<-mkWorker(traits,prior,J,pool,filt.abc,f.sumstats,pkg)(i)
