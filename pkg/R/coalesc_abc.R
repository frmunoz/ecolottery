coalesc_abc <- function(comm.obs, pool = NULL, multi = "single", prop = FALSE,
                        ...
                        migr.abc = NULL, m.replace = TRUE, size.abc = NULL, 
                        add = FALSE, var.add = NULL, params  = NULL, par.filt = NULL,
                        ...
                        method.abc = "rejection", alpha = 0.5, pkg = NULL) 
{
  ...
  initial_checks(comm.obs = comm.obs, pool = pool, multi = multi, prop = prop,
                 ...
                 size.abc = size.abc, params = params, 
                 par.filt = par.filt, par.migr = par.migr,  par.size = par.size, 
                 scale = scale, theta.max = theta.max, nb.samp = nb.samp,
  ...
  if (type == "standard")
    return(coalesc_abc_std(comm.obs, pool, multi, prop, traits, f.sumstats,
                           filt.abc, filt.vect, migr.abc, m.replace, size.abc,
                           add, var.add, params, par.filt, par.migr, par.size, 
                           constr, scale, dim.pca, svd, theta.max, nb.samp,
