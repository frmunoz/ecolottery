sumstats <- function(com, multi = "single", traits = NULL, type = "mix", n = 4,
                     m = 4)
{
  if (!requireNamespace("hillR", quietly = TRUE)) {
    stop("sumstats requires package hillR to be installed", call. = FALSE)
  }
  if (!requireNamespace("moments", quietly = TRUE)) {
    stop("function requires package moments to be installed", call. = FALSE)
  }
  
  if(!type%in%c("taxo","func","mix")) {
    stop("type should be either taxo, func or mix")
  }
  if(!multi%in%c("single","tab")) {
    stop("multi should be single or tab")
  }
  ...
  if(multi == "single"){
    ...
    if(type == "taxo"){
      if((ncol(com) < 2) & (!is.null(traits))) {
        warning("trait information is ignored when type is taxo", call. = FALSE)
      }
      tab <- table(com[,2])
      Ss <- as.vector(t(sapply(0:(n-1),
                               function(x) hillR::hill_taxa(tab, q = x))))
    }
    
    if(type == "func"){
      if(ncol(com) < 3){
        if(is.null(traits)){
          stop("trait information must be provided", call. = FALSE)
        }else{
          if(any(is.na(traits))){
            warning("Individuals with missing trait information have been ",
                    "removed", call. = FALSE)
            com <- com[-which(com[,2] == rownames(traits)[which(is.na(traits))]),] }
          
          com[,3] <- traits[as.character(com[,2]),]} } 
      if(any(is.na(com[,3]))) {
        warning("Individuals with missing trait information have been removed",
                call. = FALSE)
  ...
  if(multi == "tab"){
    ...
    if(type == "func"){
      if(is.null(traits)) stop("trait information should be provided in traits",
                               call. = FALSE)
      if(any(is.na(traits))) {
        warning("Species with missing trait information have been removed from",
                " species by site matrix/data.frame", call. = FALSE) 
