forward <- function(initial, prob = 0, D = 1, gens = 150, keep = FALSE, pool = NULL, limit.sim = F, coeff.lim.sim = 2, sigma=0.1, filt = NULL, prob.death = NULL, method.dist="euclidean")
{
   ...
   if((is.character(initial) | is.vector(initial)) & (limit.sim | !is.null(filt))) stop("Trait information must be provided along with species identity in the initial community for niche-based dynamics")
   if(!is.matrix(initial) & !is.data.frame(initial) & (limit.sim | !is.null(filt))) stop("Misdefined initial community");

   if(!limit.sim & is.null(filt)) print("Simulation of a neutral community")
   ...
   if(!is.null(pool)) if(ncol(pool)<2) stop("The regional pool is misdefined (at least two columns required when a matrix or data frame is provided)")
   else if(ncol(pool)==2) 
   {
     print("No trait information provided in the regional pool")
     if(!limit.sim | !is.null(filt)) 
   ...
   if((limit.sim | !is.null(filt)) & any(is.na(a[,3]))) stop("Trait information must be provided in initial community composition for niche-based dynamics")
...
#   }
   ...
   if(limit.sim)
   {
     if(!is.null(pool))
     {
        ...
        colnames(limit.sim) <- c(a[,1],pool[,1]); rownames(limit.sim) <- c(a[,1],pool[,1]);
     }
     else
     {
        limit.sim <- as.matrix(dist(a[,3], method=method.dist))
        colnames(limit.sim) <- a[,1]; rownames(limit.sim) <- a[,1];
     }
     diag(limit.sim) <- 0
   }
   else limit.sim <- NULL
...
{
    if(is.vector(a)) {J <- length(a); a <- data.frame(id=paste("ind",1:J,sep=""),sp=as.character(a),trait=rep(NA,J),stringsAsFactors=F);}
    else if(is.matrix(a) | is.data.frame(a)) {J <- nrow(a); if(is.matrix(a)) {a<-as.data.frame(a,stringsAsFactors=F); a[,1]<-as.character(a[,2])};}   
...
select.immigrate <- function(a, D = 1, prob.of.immigrate = 0,  pool, prob.death= NULL, limit.sim = NULL, coeff.lim.sim = 2, sigma = 0.1, filt = NULL)   # limit.sim = distances de traits; filt = fonction pour habitat filtering
{
    if(is.vector(a)) {J <- length(a); a <- data.frame(id=paste("ind",1:J,sep=""),sp=as.character(a),trait=rep(NA,J),stringsAsFactors=F);}
    else if(is.matrix(a) | is.data.frame(a)) {J <- nrow(a); if(is.matrix(a)) {a<-as.data.frame(a,stringsAsFactors=F); a[,2]<-as.character(a[,2])};}    
    else stop("select.immigrate: misdefined community composition")

    # Function defining habitat filtering according to trait value
    if(!is.null(filt)) filt1 <- function(x) filt(x)  else filt1 <- function(x) sapply(x,function(x) 1)
    ...
    else  # Influence of limiting similarity and habitat filtering on mortality
    {
        ...
        {
          if(sum(!a[,1]%in%rownames(limit.sim))>0 | sum(!a[,1]%in%colnames(limit.sim))>0) stop("limit.sim: mismatch of species names");
        ...
        if(!is.null(filt))
        {
           if(any(is.na(filt1(a[,3])))) stop("Error: NA values in habitat filtering")
    ...
    if(J1 > 0) {
        ...
        if(is.null(filt)) 
        {
          ...
          a <- rbind(a, add)
          if(sum(is.na(a[,1]))!=0) stop("Error: NA values in immigrants");
        }
        else 
        {
          a <- rbind(a, pool[sample(1:nrow(pool), J1, replace = TRUE, prob = filt1(pool[,3])),1:3])
          if(any(is.na(filt1(pool[,3])))) stop("Error: NA values in habitat filtering")
        }
        if(any(is.na(a[,1]))) stop("Error: NA values in community composition (3)");
    }
    if(J2 > 0) {
        a <- rbind(a, a.init[sample(1:nrow(a.init), J2, replace = TRUE),1:3])
        if(any(is.na(a[,1]))) {print(J2);stop("Error: NA values in community composition (4)");}
    }
    if(!is.null(limit.sim)) return(list(a=a,limit.sim.t=limit.sim.t)) else return(list(a=a))
}
