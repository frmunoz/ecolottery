get_number_of_gens = function(given_size, pool, nbrep = 100, prob = 0, d = 1, gens = NULL, limit.sim = F,
                              coeff.lim.sim = 1, sigm = 0.1, filt = NULL, prob.death = NULL,
                              method.dist = "euclidean", plot_gens = FALSE)
{
  start_com = pool[sample(1:nrow(given_pool), given_size),]
  ...
  nb_sp_gen <- data.frame(apply(nb_sp_gen, 2, function(x) tapply(x, nb_sp_gen$gens, mean)))
  
  # Make a plot to check if it is really convergent
  diagnostic_plot = ggplot(nb_sp_gen, aes(gens, rich)) +
