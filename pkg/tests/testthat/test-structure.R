...
test_that("Result structure of the forward function with limiting similarity", {
  ...
  expect_is(res[[1]], "list", info = "The com_t result (first item of the result given by the forward function) must be a list")
  expect_is(res[[1]][[1]], "data.frame", info = "Each element of the com_t result (forward function) must be data.frame")
  ...
  expect_true(sum(names(res) == c("com_t", "limit.sim.t", "pool")) == 3, info = "The result of the forward function must be named aa, limit.sim.t and pool")
})
